...
1. Skew: is a measure of the asymmetry of the probability distribution of a real-valud random variable about its mean. The skewness value can be positive or negative, or even undefined.

2. Kurtosis: measures whether the distribution is short and flat, or tall and skinny. 
	- short -> platykurtic
	- tall -> leptokurtic
	- in between -> mesokurtic

3. Modality: refers to the number of modes, or peaks, in a distribution. Real-world data is often unimodal (only had one mode)

4. Mean: is a measure of central tendency, the `.mean()` method on Pandas series can be used to calculate variance
	```python
	# The axvline function will plot a vertical line over an existing plot
	plt.axvline(value here)
	```
5. Median: is a measure of central tendency, the `.median()` method on Pandas series can be used to calculate variance.

6. Variance: how spread out the data is around the mean, the `.var()` method on Pandas series can be used to calculate variance.
$$\sigma^2 = \frac{\displaystyle\sum_{i=1}^{n}(x_i - \mathbf{\bar{x}})^2} {n}$$

7. Standard Deviation: square root of the variance. `It is typical to measure what percentage of the data is within 1 standard deviation of the mean, or two standard deviations of the mean`. `std` metnod can be used on any Pandas `Dataframe` or `Series` to calculate the standard deviation
...
12. Linear regression and **standard error**		
...
$$\sigma = \sqrt{N * p * q}$$
 ...
 - cumulative density function
	...
	```
 - z-scores: the number of standard deviations away from the mean a probability is
$$\frac{k - \mu}{\sigma}$$
...
##how to calculate P-value
 ...
 - calculate frequencies above test statistic and compare to its threshold
...
## Others
 ...
 - numpy.random.random()      Pass (32561,) into the numpy.random.random function to get a vector with 32561 elements
 - numpy.random.rand()
